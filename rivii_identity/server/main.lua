...
if Config.UseDeferrals then
    ...
    AddEventHandler('esx:playerLoaded', function(_, xPlayer)
        ...
        TriggerClientEvent('rivii_identity:alreadyRegistered', xPlayer.source)
    ...
    local function setIdentity(xPlayer)
        ...
        xPlayer.set('sex', currentIdentity.sex)
        xPlayer.set('height', currentIdentity.height)
        TriggerClientEvent('rivii_identity:setPlayerData', xPlayer.source, currentIdentity)
        if currentIdentity.saveToDatabase then
            saveIdentityToDatabase(xPlayer.identifier, currentIdentity)
        end
    ...
    local function checkIdentity(xPlayer)
        MySQL.single('SELECT firstname, lastname, dateofbirth, sex, height FROM users WHERE identifier = ?', { xPlayer.identifier }, function(result)
            if not result then
                return TriggerClientEvent('rivii_identity:showRegisterIdentity', xPlayer.source)
            end
    ...
    if not multichar then
        ...
        RegisterNetEvent('esx:playerLoaded', function(_, xPlayer)
            ...
            if currentIdentity and alreadyRegistered[xPlayer.identifier] then
                ...
                TriggerClientEvent('rivii_identity:showRegisterIdentity', xPlayer.source)
            end
        end)
    end

    ESX.RegisterServerCallback('rivii_identity:registerIdentity', function(source, cb, data)
        ...
        TriggerEvent('rivii_identity:completedRegistration', source, data)
        TriggerClientEvent('rivii_identity:setPlayerData', source, Identity)
        cb(true)
    end)
end
